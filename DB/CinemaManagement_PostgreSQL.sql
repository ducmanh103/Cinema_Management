...
-- Bản hoàn chỉnh: schema + constraints + indexes tối ưu + seed data
-- Đã sửa: ngày tháng khớp thời điểm hiện tại, cơ chế giữ ghế, FK an toàn
-- =======================================================
...
CREATE TABLE "Movies" (
    ...
    CONSTRAINT "CK_Movies_Status" CHECK ("Status" IN ('Now Showing', 'Coming Soon', 'Ended'))
);
...
-- Showtimes
-- Lưu ý: MovieId/RoomId dùng ON DELETE RESTRICT (không CASCADE) để tránh xung đột
-- với Tickets (vốn không cascade) khi cố xóa một phim/phòng đã có vé đã bán.
-- Nên xóa mềm (đổi Status Movies sang 'Ended') thay vì xóa cứng.
CREATE TABLE "Showtimes" (
    ...
    CONSTRAINT "FK_Showtimes_Movies" FOREIGN KEY ("MovieId") REFERENCES "Movies"("MovieId") ON DELETE RESTRICT,
    CONSTRAINT "FK_Showtimes_Rooms"  FOREIGN KEY ("RoomId")  REFERENCES "Rooms"("RoomId") ON DELETE RESTRICT,
    CONSTRAINT "CK_Showtimes_Price" CHECK ("Price" > 0)
);

-- Tickets
-- Cột HeldUntil: thời điểm hết hạn giữ ghế tạm (dùng cho PendingBookingCleanupService).
-- Khi Status = 'Pending' và HeldUntil < NOW() → service nền sẽ hủy vé, trả ghế lại.
CREATE TABLE "Tickets" (
    ...
    CONSTRAINT "CK_Tickets_Status" CHECK ("Status" IN ('Pending', 'Booked', 'Cancelled', 'Expired'))
);

-- Chỉ mục unique lọc: đảm bảo ở tầng database KHÔNG BAO GIỜ có 2 vé "Booked" hoặc "Pending"
-- cùng ShowtimeId + SeatId, dù ứng dụng có nhiều instance/nhiều request cùng lúc.
-- Đây là cơ chế chống double-booking khi nhiều người đặt hoặc đang thanh toán cùng 1 ghế.
CREATE UNIQUE INDEX "UQ_Ticket_Showtime_Seat" ON "Tickets"("ShowtimeId", "SeatId") WHERE "Status" IN ('Booked', 'Pending');
...
-- Lọc phim theo trạng thái (trang chủ / sắp chiếu) chạy rất thường xuyên
CREATE INDEX "IX_Movies_Status" ON "Movies"("Status");

-- Partial index riêng cho service dọn dẹp vé giữ chỗ hết hạn:
-- SELECT * FROM "Tickets" WHERE "Status"='Pending' AND "HeldUntil" < NOW();
-- Vì chỉ đánh index trên các dòng Pending, kích thước nhỏ, truy vấn cực nhanh
-- dù bảng Tickets có hàng triệu dòng lịch sử.
CREATE INDEX "IX_Tickets_Pending_HeldUntil" ON "Tickets"("HeldUntil") WHERE "Status" = 'Pending';
...
-- Movies (đã sửa: Godzilla x Kong có ReleaseDate tương lai đúng logic "Coming Soon";
-- thêm 2 phim Coming Soon mới cho tháng 10 và 12/2026)
...
('Godzilla x Kong', 115, 'Godzilla và Kong phải gạt bỏ những bất hòa xưa cũ để cùng nhau hợp tác chống lại một mối đe dọa khổng lồ mới từ bên trong Trái Đất Rỗng, đe dọa sự tồn vong của cả hai loài và toàn nhân loại.', '2026-11-20', '/img/movies/godzillaxkong.jpg', '/img/movies/godzillaxkong-banner.jpg', 'Coming Soon'),
...
('Avatar 3: Lửa và Tro Tàn', 190, 'Jake Sully và gia đình Na''vi tiếp tục cuộc chiến bảo vệ Pandora trước một bộ tộc Na''vi hung hãn mới xuất hiện.', '2026-10-16', '/img/movies/avatar3.jpg', '/img/movies/avatar3-banner.jpg', 'Coming Soon'),
('Zootopia 2', 108, 'Judy Hopps và Nick Wilde trở lại điều tra một vụ án mới, dẫn họ khám phá những ngóc ngách chưa từng biết đến của thành phố Zootopia.', '2026-12-04', '/img/movies/zootopia2.jpg', '/img/movies/zootopia2-banner.jpg', 'Coming Soon');

-- MovieGenres (đã thêm genre cho 2 phim mới: id 18, 19)
...
-- Sinh suất chiếu tự động — từ HÔM NAY (CURRENT_DATE) đến hết 31/12/2026
-- Chỉ lấy các phim đang "Now Showing"; phim "Coming Soon" chưa có suất chiếu là đúng logic.
DO $$
DECLARE
    movie_ids INT[];
    movie_count INT;
...
BEGIN
    SELECT array_agg("MovieId" ORDER BY "MovieId") INTO movie_ids
    FROM "Movies" WHERE "Status" = 'Now Showing';
...
-- =======================================================
SELECT 'Movies' AS bang, COUNT(*) AS so_dong FROM "Movies"
...
SELECT MIN("StartTime") AS suat_som_nhat, MAX("StartTime") AS suat_muon_nhat FROM "Showtimes";